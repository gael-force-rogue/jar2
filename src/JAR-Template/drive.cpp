...
Drive::Drive(enum ::drive_setup drive_setup, motor_group DriveL, motor_group DriveR,
             ...
             int SidewaysTracker_port, float SidewaysTracker_diameter, float SidewaysTracker_center_distance) : wheel_diameter(wheel_diameter),
...
void Drive::set_coordinates(float X_position, float Y_position, float orientation_deg) {
    ...
    odom_task = vex::thread(position_track_task);
}
...
void Drive::arcade(float y, float x) {
    DriveL.spin(fwd, to_volt(y + x), volt);
    DriveR.spin(fwd, to_volt(y - x), volt);
}
...
void Drive::tank(float left, float right) {
    DriveL.spin(fwd, to_volt(left), volt);
    DriveR.spin(fwd, to_volt(right), volt);
};
